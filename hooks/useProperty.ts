...
const useProperty = () => {
  ...
  const { get: query, post, put: putMutation, delete: delProperty, patch } = useApiUrl();
  ...
  const getAdminProperty = useCallback(
    async (inputValue: string, page: number) => {
      try {
        const res = await query(`/property/admin?keyword=${inputValue}&page=${page}`);
  ...
  const getFewAdminProperty = useCallback(async () => {
    try {
      const res = await query(`/property/few-admin`);
      return res as PropertyResponse;
    } catch (err: any) {
      throw new err();
    }
  ...
  const getFewVerifiedProperty = useCallback(async () => {
    try {
      const res = await query(`/property/few-verified`);
      return res as PropertyResponse;
    } catch (err: any) {
      throw new err();
    }
  ...
  const getVerifiedProperty = useCallback(
    async (inputValue: string, page: number) => {
      try {
        const res = await query(`/property/verified-properties?keyword=${inputValue}&page=${page}`);
        return res as PropertyResponse;
      } catch (err: any) {
        throw new err();
      }
  ...
  const getVerifiedProjects = useCallback(
    async (page: number) => {
      try {
        const res = await query(`/property/verified-projects?page=${page}`);
        // console.log('res', res);
        return res as PropertyResponse;
      } catch (err: any) {
        throw new err();
