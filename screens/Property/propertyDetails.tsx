...
import { DeleteProperty, VerifyState, DeclineState, SoldState } from './VerificationState';
...
export const PropertyDetails = ({ my, p, cardWidth }: { my?: string; p?: string; cardWidth?: any }) => {
  ...
  const [verificationStatus, setVerificationStatus] = useState(detailsData?.verification);
  const defaultLayoutPluginInstance = defaultLayoutPlugin();
  const { toast } = useToast();
  const router = useRouter();

  const { getPropertyDetails, verifyProperty, deleteProperty, getFewAdminProperty } = useProperty();
  ...
  const getPropertyFunction = async () => {
    ...
    try {
      setisLoading(false);
      const getAllProperties = await getFewAdminProperty();
  ...
  const changeStateFxn = async (status: string) => {
    if (!id) {
      toast({
        status: 'error',
    ...
    try {
      ...
      if (req.status === 201) {
        ...
        toast({
          status: 'success',
          description: `Property ${req.data.data}`,
          title: 'Success',
          position: 'top',
          duration: 1000,
        });
  ...
  useEffect(() => {
    getPropertyDetailFn();
  }, [id, showModal, isLoading]);

  return (
    <>
      <Modal closeOnOverlayClick={true} onClose={toggleModal} isOpen={activeModal}>
        <ModalOverlay />
        <ModalContent>
          {modalType === 'Verified' ? (
            <VerifyState changeStateFxn={changeStateFxn} isLoading={isLoading} toggleModal={toggleModal} />
          ) : modalType === 'Rejected' ? (
            <DeclineState toggleModal={toggleModal} isLoading={isLoading} changeStateFxn={changeStateFxn} />
          ) : modalType === 'Sold' ? (
            <SoldState toggleModal={toggleModal} isLoading={isLoading} changeStateFxn={changeStateFxn} />
          ) : modalType === 'delete' ? (
            <DeleteProperty toggleModal={toggleModal} isLoading={isLoading} deletePropertyFn={deletePropertyFn} />
        ...
        {!loading && detailsData && (
          <>
            <Flex w={'100%'} my={my || '24px'} pos={'relative'}>
              <Grid
                templateColumns={detailsData?.images.length === 1 ? '' : 'repeat(2, 1fr)'}
                gap={'16px'}
                w={'100%'}
                h={'max-content'}
              >
            ...
            <Flex flexDir={'column'} w={'100%'} p={p || '20px'} gap={'24px'}>
              ...
              >
                ...
                <Text fontWeight={500} display={'flex'} alignItems={'center'}>
                  <TbCurrencyNaira />
                  <Text as={'span'}>{Number(detailsData?.price?.amount)?.toLocaleString()}</Text>
                </Text>
              </Flex>
              <Flex flexDir={{ base: 'column', lg: 'row' }} w={'100%'} justifyContent={'space-between'} gap={'26px'}>
                <Flex flexDir={'column'} w={'100%'} gap={'18px'} className="roboto">
                  ...
                  <Text textColor={'#626871'} fontWeight={400} fontSize={'18px'} className="roboto">
                  ...
                  <Flex
                    ...
                    className="robotoF"
                  >
                    <Text fontSize={'12px'} fontWeight={500} textColor={'var(--soft400)'}>
                      DOCUMENTS
                    </Text>
                  </Flex>
                ...
                <Flex flexDir={'column'} w={{ base: '100%', lg: '20%' }} gap={'16px'} className="robotoF">
                  {detailsData?.ownerID?.role === 'ADMIN' ? (
                    verificationStatus === 'Pending' ? (
                      ...
                      <Flex gap={'16px'} direction={'column'}>
                        {(verificationStatus === 'Verified' || verificationStatus === 'Sold') && (
                          <Box>
                  ...
                  >
                    <Text fontSize={'12px'} fontWeight={500} textColor={'var(--soft400)'}>
                      {detailsData?.ownerID?.role}
                    </Text>
                  </Flex>
                  <Flex flexDir={'column'} gap={'16px'} w={'100%'} p={'20px'} className="robotoF">
                    <Flex alignItems={'center'} gap={4}>
                      <Box overflow={'hidden'} borderRadius={'100%'} h={'40px'} w={'42px'}>
                        <Image
                          src={detailsData?.ownerID?.avatar || '/profile.png'}
                          alt=""
                          h={'40px'}
                          w={'50px'}
                          borderRadius={'50%'}
                      ...
                      <Text fontWeight={500} fontSize={'18px'}>
                        {`${detailsData?.ownerID?.firstName} ${detailsData?.ownerID?.lastName}`}
                      </Text>
                    </Flex>
                    <Box textColor={'#626871'} fontWeight={500}>
                      ...
                      <Text fontSize={'14px'}>{`${detailsData?.ownerID?.email}`}</Text>
                    </Box>
                    <Flex w={'100%'} gap={'28px'}>
                      <Box textColor={'#626871'} fontWeight={500}>
                        <Text mb={'4px'} textColor={'var(--soft400)'} fontSize={'12px'}>
                        ...
                        <Text fontSize={'14px'}>{detailsData?.ownerID?.phoneNumber}</Text>
                      </Box>
                      <Box textColor={'#626871'} fontWeight={500}>
                        <Text mb={'4px'} textColor={'var(--soft400)'} fontSize={'12px'}>
                          USER TYPE
