...
const DashboardScreen = () => {
  ...
  const { getFewVerifiedProperty } = useProperty();

  const getPropertyFunction = async () => {
    setLoading(true);
    try {
      const getAllProperties = await getFewVerifiedProperty();
  ...
  return (
    <Box w={'100%'}>
        ...
        >
          <Flex w={'fit-content'} flexDir={'column'}>
            <Flex alignItems={'center'} gap={'24px'}>
              <Text fontWeight={500} fontSize={'15px'} className="montserrat" textColor={'#172B4C'}>
                {PropertiesCount}
              </Text>
            </Flex>
            <Text fontWeight={400} fontSize={'18px'} className="robotoF" textColor={'#808080'}>
              Properties
            </Text>
          </Flex>
      ...
      <Flex gap={'20px'} w={'100%'}>
        <Flex w="100%" justifyContent={'space-between'} alignItems={'center'}>
          <Box w={'100%'} className="robotoF" mb={'24px'}>
            <Text fontWeight={500} textColor={'var(--strong950)'} fontSize={{ base: '14px', lg: '18px' }}>
              NEW USERS
            </Text>
            <Text fontWeight={400} textColor={'var(--sub600)'} fontSize={{ base: '10px', lg: '14px' }}>
      ...
      <TableContainer mt="30px">
        {!loadingUser && table?.length > 0 && (
          <Table size="sm">
            <Thead>
              <Tr bgColor={'#F5F7FA'}>
                {['Full Name', 'Email', 'Phone No.', 'Date Created', 'Property', 'User Type'].map((item, key) => (
                  <Th key={key} textTransform={'none'} p="8px">
                    <Text className="robotoF" color={'#525866'} fontWeight={400} fontSize={'.875rem'}>
                      {item}
                    </Text>
                  </Th>
                ))}
            ...
            <Tbody fontSize={'.875rem'} fontWeight={400} className="robotoF">
              {table &&
                table.slice(0, 5).map((item: any) => (
                  <Tr key={item._id} cursor={'pointer'} onClick={() => openDrawer(item._id)}>
                    <Td color={'#0E121B'} py="12px">{`${item.firstName.slice(0, 1).toUpperCase()}${item.firstName.slice(
                      1,
                      item.firstName.length
                    )} ${item.lastName.slice(0, 1).toUpperCase()}${item.lastName.slice(1, item.firstName.length)}`}</Td>
              ...
              <UserDrawer isOpen={isOpen} onClose={onClose} btnRef={btnRef} userEl={userEl} propCount={propCount} />
            </Tbody>
          </Table>
        )}
        {loadingUser && (
