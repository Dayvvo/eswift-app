...
class BlogPostController {
  ...
  updatePost = async (req: Request, res: Response) => {
    ...
    try {
      ...
      const blogPost = await BlogPost.findOneAndUpdate({ _id: req.params.blogPostId }, { ...value }, { new: true });
  ...
  fetchBlogPost = async (req: Request, res: Response) => {
    ...
    try {
      ...
      const blogpost = await BlogPost.find(findQuery).lean().sort({ createdAt: -1 }).skip(skip).limit(POST_PER_PAGE);

      const modifiedBlogPosts = blogpost.map((post) => {
        return {
          ...post,
          header_image:
            post.header_image && !(post.header_image.startsWith('http') || post.header_image.startsWith('https'))
              ? `${process.env.BACKEND_URL}/uploads/${post.header_image}`
              : post.header_image,
          body_image:
            post.body_image && !(post.body_image.startsWith('http') || post.body_image.startsWith('https'))
      ...
      return res.status(200).json({
        ...
        pagination: {
          totalPost: totalPost,
          page: page,
          pages: Math.ceil(totalPost / POST_PER_PAGE),
        },
      });
    } catch (error) {
  ...
  loadBlogPost = async (req: Request, res: Response) => {
    const page = parseInt(req.query.page as string) || 1;
    ...
    try {
      ...
      const blogpost = await BlogPost.find().lean().sort({ createdAt: -1 }).skip(skip).limit(POST_PER_PAGE);
      ...
      return res.status(200).json({
        ...
        paginate: {
          page,
          totalPost,
          totalPage: Math.ceil(totalPost / POST_PER_PAGE),
        },
      });
    } catch (error) {
      res.status(500).send('Failed to fetch blog posts');
